#!/usr/bin/env python3
"""
Generate object detection and tracking data for dart_supervision testing.

This script runs YOLO11n on people-walking.mp4, applies ByteTracker tracking,
and saves the results as normalized coordinates in JSON format.

Usage:
    uv run python generate_detections.py

Output:
    assets/people-walking-detections.json - Detection data with normalized coordinates
"""
...
def normalize_detections(detections: sv.Detections, frame_width: int, frame_height: int) -> dict:
    ...
    result["confidence"] = detections.confidence.tolist() if detections.confidence is not None else [1.0] * len(detections)
    result["class_id"] = detections.class_id.tolist() if detections.class_id is not None else [0] * len(detections)
    result["tracker_id"] = detections.tracker_id.tolist() if detections.tracker_id is not None else list(range(len(detections)))
...
def main():
    ...
    for frame_idx, frame in enumerate(sv.get_video_frames_generator(str(video_path))):
        ...
        normalized = normalize_detections(detections, video_info.width, video_info.height)
        results.append({
        ...
        if frame_idx % 30 == 0:  # Print progress every 30 frames
            print(f"Processed frame {frame_idx}/{video_info.total_frames or 'unknown'} - {len(detections)} detections")
