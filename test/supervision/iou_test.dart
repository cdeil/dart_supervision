...
void main() {
  group('IoU Calculation', () {
    test('calculates IoU correctly for overlapping boxes', () {
      final box1 = sv.NDArray([4], data: [0.0, 0.0, 10.0, 10.0]); // 100 area
      final box2 = sv.NDArray(
        [4],
        data: [5.0, 5.0, 15.0, 15.0],
      ); // 100 area, 25 overlap

      final iou = sv.IoUCalculator.iou(box1, box2);
      expect(iou, closeTo(25.0 / 175.0, 0.001)); // intersection / union
    });

    test('returns 0 for non-overlapping boxes', () {
      final box1 = sv.NDArray([4], data: [0.0, 0.0, 5.0, 5.0]);
      final box2 = sv.NDArray([4], data: [10.0, 10.0, 15.0, 15.0]);
    ...
    test('returns 1 for identical boxes', () {
      final box1 = sv.NDArray([4], data: [0.0, 0.0, 10.0, 10.0]);
      final box2 = sv.NDArray([4], data: [0.0, 0.0, 10.0, 10.0]);
    ...
    test('computes IoU matrix between box sets', () {
      ...
      final iouMat = sv.IoUCalculator.iouBatch(boxes1, boxes2);
    ...
    test('computes IoU distance matrix', () {
      final boxes1 = sv.NDArray([1, 4], data: [0.0, 0.0, 10.0, 10.0]);
      final boxes2 = sv.NDArray([1, 4], data: [0.0, 0.0, 10.0, 10.0]);

      final distances = sv.IoUCalculator.iouDistance(boxes1, boxes2);
